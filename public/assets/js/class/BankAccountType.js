...
const CONFIG = {
    selectors: {
        table: '#bank-account-type-table',
        tableColumn: '#bank-account-type-table-column-dropdown',
        form: '#bank_account_type_form',
        formId: 'bank_account_type_form',
        detailId: 'bank_account_type_id',
        ...
        filterCollapse: 'bank-account-type-filter-collapse',
    ...
    endpoints: {
        tableData: '/bank-account-type/generate-table',
        save: '/bank-account-type/save',
        delete: '/bank-account-type/delete',
        deleteMultiple: '/bank-account-type/delete-multiple',
        fetch: '/bank-account-type/fetch',
    }
};
...
export class BankAccountType {
    ...
    async init() {
        return PageInitializer.run(async () => {
            ...
            AuditLogManager.attachLogNotesClassHandler(CONFIG.selectors.logNotesTrigger, 'bank_account_types');
        });
    }
    ...
    initDelete() {
        initConfirmAction({
            ...
            method: 'DELETE',
            payload: { bank_account_type_id: (el) => el.dataset.referenceId },
    ...
    async handleFetchWorkflow(referenceId) {
        await DetailFetcher.fetch({
            ...
            onSuccess: async (response) => {
                ...
                const targetFields = {
                    'bank_account_type_id': referenceId,
                    'name': data.name,
                };
