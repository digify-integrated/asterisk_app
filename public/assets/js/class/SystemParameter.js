...
const CONFIG = {
    selectors: {
        table: '#system-parameter-table',
        tableColumn: '#system-parameter-table-column-dropdown',
        form: '#system_parameter_form',
        formId: 'system_parameter_form',
        detailId: 'system_parameter_id',
        ...
        filterCollapse: 'system-parameter-filter-collapse',
    ...
    endpoints: {
        tableData: '/system-parameter/generate-table',
        save: '/system-parameter/save',
        delete: '/system-parameter/delete',
        deleteMultiple: '/system-parameter/delete-multiple',
        fetch: '/system-parameter/fetch',
    }
};
...
export class SystemParameter {
    ...
    async init() {
        return PageInitializer.run(async () => {
            ...
            AuditLogManager.attachLogNotesClassHandler(CONFIG.selectors.logNotesTrigger, 'system_parameters');
        });
    }
    ...
    initDelete() {
        initConfirmAction({
            ...
            method: 'DELETE',
            payload: { system_parameter_id: (el) => el.dataset.referenceId },
    ...
    async handleFetchWorkflow(referenceId) {
        await DetailFetcher.fetch({
            ...
            onSuccess: async (response) => {
                ...
                const targetFields = {
                    'system_parameter_id': referenceId,
                    'name': data.name,
