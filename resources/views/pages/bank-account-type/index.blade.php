    ...
    <div class="card border-0 shadow-sm mb-3">
        <div class="card-body py-4">
            <div class="d-flex flex-column flex-xl-row align-items-xl-center justify-content-between gap-4">
                ...
                <div class="d-flex align-items-center justify-content-end flex-wrap gap-2">
                    ...
                    @component('partials.filter-button')
                        @slot('collapseId', 'bank-account-type-filter-collapse')
                    @endcomponent
                    ...
                    @component('partials.column-dropdown')
                        @slot('dropdownId', 'bank-account-type-table-column-dropdown')
                        @slot('dropdownButtonId', 'bank-account-type-table-button-column-dropdown')
                    @endcomponent
                </div>
            </div>
            ...
            @component('partials.filter-module')
                @slot('collapseId', 'bank-account-type-filter-collapse')
                @slot('resetFilterId', 'bank-account-type-reset-filters-btn')
                @slot('applyFilterId', 'bank-account-type-apply-filters-btn')
            @endcomponent
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-body pt-3 pb-3 pe-0 ps-0">
            @component('partials.index-table')
                @slot('tableId', 'bank-account-type-table')
            @endcomponent
        </div>

        @if($pageType == 'single_page')
            @component('partials.form-modal')
                @slot('formTitle', 'Bank Account Type Details')
                @slot('formId', 'bank_account_type_form')
                @slot('size', 'md')

                <input type="hidden" id="bank_account_type_id" name="bank_account_type_id" />
