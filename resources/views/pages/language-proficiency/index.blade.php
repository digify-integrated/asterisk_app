    ...
    <div class="card border-0 shadow-sm mb-3">
        <div class="card-body py-4">
            <div class="d-flex flex-column flex-xl-row align-items-xl-center justify-content-between gap-4">
                ...
                <div class="d-flex align-items-center justify-content-end flex-wrap gap-2">
                    ...
                    @component('partials.filter-button')
                        @slot('collapseId', 'language-proficiency-filter-collapse')
                    @endcomponent
                    ...
                    @component('partials.column-dropdown')
                        @slot('dropdownId', 'language-proficiency-table-column-dropdown')
                        @slot('dropdownButtonId', 'language-proficiency-table-button-column-dropdown')
                    @endcomponent
                </div>
            </div>
            ...
            @component('partials.filter-module')
                @slot('collapseId', 'language-proficiency-filter-collapse')
                @slot('resetFilterId', 'language-proficiency-reset-filters-btn')
                @slot('applyFilterId', 'language-proficiency-apply-filters-btn')
            @endcomponent
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-body pt-3 pb-3 pe-0 ps-0">
            @component('partials.index-table')
                @slot('tableId', 'language-proficiency-table')
            @endcomponent
        </div>
